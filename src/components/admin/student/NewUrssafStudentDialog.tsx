          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-12 pb-12">
              ...
              {Object.keys(form.formState.errors).length > 0 && (
                <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl text-amber-800">
                    ...
                    {(() => {
                      const getErrors = (obj: any, path: string[] = []): { path: string; message: string }[] => {
                        ...
                        if (typeof obj === 'object' && 'message' in obj && typeof obj.message === 'string') {
                          const pathStr = path
                            .map(p => {
                              if (p === 'studentRate') return 'Tarif';
                              if (p === 'courseDay') return 'Jour de cours';
                              if (p === 'courseHour') return 'Heure de cours';
                              if (p === 'notes') return 'Notes';
                              if (p === 'address') return 'Adresse';
                              if (p === 'urssaf') return 'URSSAF';
