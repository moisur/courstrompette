    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-stone-500">
          Chaque soumission est conservee en base. Tu peux suivre le statut mail et convertir un lead en eleve.
        </p>
      </div>
              ...
              leads.map((lead: LeadRecord) => (
                <TableRow key={lead.id}>
                  ...
                  <TableCell className="min-w-[140px]">
                    <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wide ${getStageStyles(lead.stage)}`}>
                      {lead.stage}
                    </span>
                  </TableCell>
                  <TableCell className="min-w-[180px]">
                    <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wide ${getMailStyles(lead.mailStatus)}`}>
                      {lead.mailStatus}
                    </span>
                    {lead.mailError ? (
